...
#include basics\setVars.ahk
#include basics\backupsavefile.ahk

global gameWindowId := "ahk_exe eu4.exe"
global settings := readSettings("settings.ini", settingsFile)

logFileName := StrReplace(settingsFile, "settings-", "")
logFileName := StrReplace(logFileName, ".ini", "")
global logfile := "log-" . logFileName . ".txt"
...
readSettings(baseSettingsFile, settingsFile) {
    ...
    return settings
} ;readsettings thanks to joffreybesos source: https://github.com/joffreybesos/d2r-baalpassenger/blob/master/src/init.ahk
